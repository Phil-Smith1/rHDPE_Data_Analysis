...
def produce_full_dataset_of_features( root_directory, dataset, sample_mask ):
...
def pca( directory, features_df, std_of_features_df ):
    ...
    if pca_of_whole_dataset:
        ...
        gu.perform_pca( directory, features_df, [int( i ) for i in features_df.index], std_error = True, std_of_features_df = std_of_features_df, num_components = 2, filename = directory + "www/Output/PCA/Overall.png" )

    if pca_of_pca_of_individual_datasets:

        compute_distance_to_virgin = True

        perform_k_means = True
        ...
        pca_ft_df = gu.perform_pca( directory, overall_pca_df, [int( i ) for i in overall_pca_df.index], std_error = True, std_of_features_df = overall_stds_df, num_components = 3, filename = output_directory + "PCA/Overall.pdf" )
